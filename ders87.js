...
let removeAllTodosButton = document.querySelector("#todoClearButton");
...
function removeAllTodos(e){
  ...
  if(todos.length > 0){
    todos.forEach(function(todo){
      todo.remove();
      removeAllTodosFromStorage(todo);
    });
  }else{
    showAlert("warning" , "Silmek ucun en az 1 todo olmalidir.");
  }
  console.log(todos);
}

function removeAllTodosFromStorage(todo){
  todoStorage = [];
  localStorage.setItem("todoStorage" , JSON.stringify(todoStorage));
  showAlert("success" , "All todos deleted succesfully");
}
